...
$('#btnsubmit').click(function() {
  ...
  if ( $('#input-phone').val().length < 7 ) {
    ...
    // підсвітка input'а та фокус
    $('.regform__phone-wrapper:eq(0)').css({'box-shadow':'0 0 5px red','border-color':'red'}).focus();
...
$('#input-lastName').keyup(function(e){
  if ( $('#input-lastName').val().length > 1 ) {
    ...
    isMessageVisible = false;
    // підсвітка input'а
    $('#input-lastName').css({'box-shadow':'none','border-color':'white'});
  }
});
...
$('#input-phone').keyup(function(e){
  if ( $('#input-phone').val().length > 6 ) {
    ...
    isMessageVisible = false;
    // підсвітка input'а
    $('.regform__phone-wrapper:eq(0)').css({'box-shadow':'none','border-color':'white'});
  }
});
...
$('#input-password, #input-confirm-pass').keyup(function(){
  ...
  if ( tempPass.length >= 8 && tempPass.length != tempCount && tempCount != 0 ) {
    ...
    isMessageVisible = false;
    // підсвітка input'а
    $('#input-password, #input-confirm-pass').css({'box-shadow':'none','border-color':'white'});
  }
});
...
$('.regform__agree-label').click(function(){
  ...
  if ( "attr('checked')", $('.regform__agree-wrapper input:checked').length == 1 ) {
    ...
    isMessageVisible = false;
    // підсвітка input'а
    $('.regform__agree-checkbox-span').css({'box-shadow':'none','border-color':'white'});
  }
});
...
function showErrorMessage(query, text, top, left) {
  ...
  let message      = $('<div class="regform__error"></div>').text(text[currentLang])
